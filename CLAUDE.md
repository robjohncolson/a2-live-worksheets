...
Content is authored in `../Lesson_planning`; see `A2_FORK_PLAN.md` for the Savvas transcription and question-bank pipeline. Lessons 1-1, 1-2, 1-5 and 1-6 are published in `content/a2/` (two-week windows; `docs/a2-lesson-targets.md` sets the authoring order); other lessons stay unpublished until authored content is ready. After editing `content/a2/lessons.json` run `node scripts/sync-server-shared.mjs`, `node scripts/build-a2-roadmap-data.mjs` and `node scripts/build-a2-work-manifest.mjs`; after any Desk/JS/CSS/content change run `node scripts/bump-build.mjs` before committing (PWA cache).
