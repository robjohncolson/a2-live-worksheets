...
const averageDay = section('An average day').querySelector('p').outerHTML;
const graded = section('What gets graded');
const computed = [...doc.querySelectorAll('#how-your-grade p')].map(p => p.outerHTML).join('\n');
const addDrop = section('Right now (through Fri Sep 18)').querySelector('p').outerHTML;
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .pacing td:first-child { width: 24%; } .pacing td:last-child { width: 13%; white-space: nowrap; }
...
${graded.querySelector('table').outerHTML}
${computed}
<h2>Bonus</h2>
${bonus}
...
<p>Each date is the day that lesson is due for Period ${period}. These are targets; I move them when the class needs more or less time, and the Desk calendar always shows the current dates.</p>
<table class="pacing"><thead><tr><th>Topic</th><th>Lessons (due)</th><th>Topic test</th></tr></thead><tbody>
${pacingRows(period)}
</tbody></table>
<p>Quarters close ${quarters}</p>
<h2>Where to look</h2>
<p>The Desk: sign in with your username and PIN on your Chromebook. It shows your scores, your live grade, and what is due.</p>
<p>Schoology: the OneNote notebook link, the week's IXL focus folder, this syllabus, and grades.</p>
<p>Families: message me through ParentSquare.</p>
<nav aria-label="Syllabus actions"><a class="btn" href="desk.html">Open the Algebra 2 Desk</a><button type="button" onclick="window.print()">Print syllabus</button></nav>
</main></body>
</html>
`;
}
